...
$libraryIds = $standardUser ? fetch_library_film_ids($pdo, (int) $currentUser['id']) : [];
$libraryIdLookup = array_flip($libraryIds);
$libraryPreview = $standardUser ? fetch_user_library($pdo, (int) $currentUser['id'], 5) : [];
$libraryCount = $standardUser ? count_library_items($pdo, (int) $currentUser['id']) : 0;

$pageTitle = 'Svijet filmova';
$pageHeading = 'Svijet filmova';
$pageDescription = 'PHP videoteka s prijavom korisnika, filtriranjem filmova i trajnim spremanjem osobne videoteke.';
$styles = ['style.css'];
$activePage = 'films';
...
<?php elseif (is_admin()): ?>
                    <span class="muted-text">Admin upravlja filmovima</span>
<?php else: ?>
...
<?php elseif (is_admin()): ?>
        <div class="empty-state">
          <p>Administratorski račun nema osobnu videoteku. Za ovaj račun dostupno je upravljanje filmovima i fotografijama.</p>
          <div class="action-row">
            <a class="primary-link-button" href="dashboard.php">Otvori administraciju</a>
          </div>
        </div>
<?php else: ?>
        <div class="empty-state">
